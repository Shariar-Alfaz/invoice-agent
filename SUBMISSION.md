...
- **How you would find out if something was registered incorrectly**: Keep a simple audit record for each invoice: the original file, extracted data, reviewer approval, and accounting response. Then compare the invoices we submitted with the invoices in the accounting system. I would flag duplicate invoice numbers, changed totals, changed supplier codes, and any invoice that needed many manual corrections before submit.
