...
- **Cost per invoice** (and what makes it up): For production, I would budget for a stronger OCR/document extraction service plus an LLM. A practical accuracy-first stack is Google Document AI Form Parser or Custom Extractor plus Gemini. Document AI Form Parser/Custom Extractor is about `$30` per 1,000 pages, or `$0.030` for a one-page invoice. If each invoice uses about 5,000 input tokens and 1,000 output tokens, Gemini 3.1 Pro Preview is about `$0.022` per invoice. Total estimate: about `$0.052` per one-page invoice before hosting, storage, and reviewer labor.
- **Monthly cost at 1,000 invoices per month**: With the accuracy-first stack, 1,000 one-page invoices would cost about `$52/month`: `$30` for Document AI and `$22` for Gemini 3.1 Pro Preview. A cheaper balanced option with Gemini 3.6 Flash would be about `$37.50/month` through December 31, 2026, and about `$45/month` from January 1, 2027. Raw OCR alone can be much cheaper, around `$1.50` per 1,000 pages with Google Enterprise Document OCR, but I would not rely on raw OCR alone for messy invoice tables.
