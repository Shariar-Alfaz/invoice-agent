...
The accounting API is isolated behind an async `httpx` client. The base URL and API key come from settings. The client supports `/health`, `/partners`, `/tax-codes`, `/invoices`, and `POST /invoices`. It preserves accounting API error codes and surfaces them in the processing result.
...
- **Cost per invoice** (and what makes it up): For production I would budget against an accuracy-first stack: Google Document AI Form Parser or Custom Extractor for invoice/table extraction plus Gemini for structured reasoning and cleanup. Google Document AI Form Parser/Custom Extractor is $30 per 1,000 pages, so a one-page invoice costs about `$0.030` for OCR/document extraction. Assuming each invoice sends about 5,000 input tokens and receives 1,000 output tokens, Gemini 3.1 Pro Preview costs about `$0.022` per invoice (`5,000 * $2 / 1M + 1,000 * $12 / 1M`). Estimated total: about `$0.052` per one-page invoice before storage, hosting, and reviewer labor.
- **Monthly cost at 1,000 invoices per month**: Accuracy-first production estimate: Document AI `$30.00` + Gemini 3.1 Pro Preview `$22.00` = about `$52.00/month` for 1,000 one-page invoices. A cheaper balanced option using Gemini 3.6 Flash is about `$37.50/month` through December 31, 2026: Document AI `$30.00` + Gemini `$7.50` (`5M input tokens * $0.75/1M + 1M output tokens * $3.75/1M`). From January 1, 2027, the same Gemini 3.6 Flash volume would be about `$15.00`, making the total about `$45.00/month`. If only raw OCR is needed, Google Enterprise Document OCR is much cheaper at `$1.50` per 1,000 pages, but I would not treat raw OCR alone as the best production option for messy invoice tables.
