...
- **Cost per invoice** (and what makes it up): For production, I would budget for a stronger OCR/document extraction service plus an LLM. I assume one invoice is one page, and each LLM call uses about 5,000 input tokens and 1,000 output tokens. Google Document AI Form Parser or Custom Extractor is about `$30` per 1,000 pages, or `$0.030` per invoice. On top of that, the LLM cost would be about `$0.00325` with OpenAI GPT-5 mini, `$0.01625` with OpenAI GPT-5, about `$0.0075` with Gemini 3.6 Flash introductory pricing, or about `$0.022` with Gemini 3.1 Pro Preview. So the total estimated cost per one-page invoice is about `$0.03325` with GPT-5 mini, `$0.04625` with GPT-5, `$0.0375` with Gemini 3.6 Flash, or `$0.052` with Gemini 3.1 Pro Preview, before hosting, storage, and reviewer labor.
- **Monthly cost at 1,000 invoices per month**: With Document AI plus OpenAI GPT-5 mini, the estimate is about `$33.25/month`: `$30.00` for Document AI and `$3.25` for the LLM. With Document AI plus OpenAI GPT-5, it is about `$46.25/month`: `$30.00` for Document AI and `$16.25` for the LLM. With Document AI plus Gemini 3.6 Flash, it is about `$37.50/month` through December 31, 2026, and about `$45.00/month` from January 1, 2027. With Document AI plus Gemini 3.1 Pro Preview, it is about `$52.00/month`. I would start production evaluation with GPT-5 mini or Gemini 3.6 Flash, then use the stronger model only for low-confidence or disputed invoices if accuracy requires it. Raw OCR alone can be much cheaper, around `$1.50` per 1,000 pages with Google Enterprise Document OCR, but I would not rely on raw OCR alone for messy invoice tables.
